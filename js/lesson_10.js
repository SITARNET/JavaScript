...
// и clientY равны 0. А если мышь находится в центре окна, то значения clientX и clientY равны 250 независимо от того,
// в каком месте документа она находится и до какого места документ прокручен. В этом они похожи на position:fixed.

// Наведите курсор мыши на поле ввода, чтобы увидеть clientX/clientY (пример находится в iframe, поэтому координаты
// определяются относительно этого iframe):
...
// Подробнее о координатах вы можете узнать в главе Координаты.
...
// Если зажать левую кнопку мыши и, не отпуская кнопку, провести мышью, то также будет выделение, которое в интерфейсах
// может быть «не кстати».

// Есть несколько способов запретить выделение, о которых вы можете прочитать в главе Selection и Range.

// В данном случае самым разумным будет отменить действие браузера по умолчанию при событии mousedown, это отменит оба
// этих выделения:
...
// Заметим, что текст внутри него по-прежнему можно выделить, если начать выделение не на самом тексте, а до него или после.
// Обычно это нормально воспринимается пользователями.
...
// Если вы попытаетесь скопировать текст в <div>, у вас это не получится, потому что срабатывание события oncopy по
// умолчанию запрещено.
...
// Если вы планируете обработать Ctrl, то не забудьте, что пользователи Mac обычно используют Cmd, поэтому лучше
// проверить if (e.metaKey || e.ctrlKey).
...
// В следующей главе мы поговорим о событиях, которые возникают при передвижении мыши, и об отслеживании смены
// элементов под указателем.
...
// Если клик сделан вместе с Ctrl (Cmd для Mac), то выделение переключается на элементе, но остальные элементы при
// этом не изменяются.
...
function toggleSelect(li) {
    li.classList.toggle('selected'); // если есть, то скрывает, если нет, то открывает
}

function singleSelect(li) {
    let selected = ul.querySelectorAll('.selected');
    for(let elem of selected) {
        elem.classList.remove('selected'); // удаляет
    }
    li.classList.add('selected'); // добавляет
}