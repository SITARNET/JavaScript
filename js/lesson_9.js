...
// Для новых, пользовательских событий браузерных действий, конечно, нет, но код, который генерирует такое событие, может
// предусматривать какие-то свои действия после события.
// Вызов event.preventDefault() является возможностью для обработчика события сообщить в сгенерировавший событие код, что
// эти действия надо отменить.
...
// Ниже вы можете видеть кролика #rabbit и функцию hide(), которая при вызове генерирует на нём событие "hide", уведомляя
// всех интересующихся, что кролик собирается спрятаться.
